...
export {storage,gallery,order,customer,votes,indecators,users,answers, firebase as default}
...
update_customer=(id,name,price,tittle,email,name_of_product,size,country,city,address,count)=>{   
    customer.child(id).update({'name':name,'total':price,'tittle':tittle,'email':email
    ,'name_of_product':name_of_product,'size':size,'country':country,'city':city,'address':address,'count':count})
}
    getItem=id=>{
...
addToCart=(id,price)=>{
    ...
    addToCart_from_details=(id,size_state,price_size,original_price,name)=>{
        let tempProducts=[...this.state.products];
        const index=tempProducts.indexOf(this.getItem(id));
        const product=tempProducts[index];
        product.inCart=true;
    ...
    this.setState(()=>{
        ...
        change_cart_size = (id,price,size,original_price) => {
            ...
            this.setState(prevState => ({
                ...
                obj => (obj.id === id ? Object.assign(obj, { dropdown:!obj.dropdown, price: new_price , total:new_price , count:1,size:size }) : obj)
              )
            }), ()=>{
                this.addTotal(); 
            })}
...
dropdown_buttun = (id) => {
    ...
    nextItem_for_second_question=(answer_second)=>{
        users.child(this.state.user[0].id).child('org_form').set(answer_second);
        this.setState({
            number:this.state.number+1
        })
    }
    nextItem=(id,name,answer,indecator,weight,weight_option_state)=>{
        ...
        if (this.state.number === length - 1){
            ...
            var followers = {};
            gallery.child(id).child('data').child(name).on('value', (snap) => followers = snap.val()); 
            gallery.child(id).child('data').child(name).set(followers+1)
            var in_oblast = {};
            gallery.child(id).child(this.state.user[0].oblast).child(name).on('value', (snap) => in_oblast = snap.val()); 
            gallery.child(id).child(this.state.user[0].oblast).child(name).set(in_oblast+1)
    ...
    do_login = (login,password) =>  {
        const user=[];
        users.orderByChild("login").equalTo(login).once("value", snapshot => {
            if (snapshot.exists()){
                users.orderByChild("login").equalTo(login).once('child_added').then( (snap) =>  {
                    var currentdate = new Date(); 
                   var datetime = (currentdate.getMonth()+1) + "/"
                                   + currentdate.getDate() + "/" 
                                   + currentdate.getFullYear();
    ...
    change_Option=(indecator)=>{
        ...
        gallery.orderByChild('indecator').equalTo(indecator).on('child_added', snap => {
