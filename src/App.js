...
import  { useState } from 'react';
 ...
 const APP=()=>{
    const [dropdown, set_dropdowb] = useState(false);
        ...
        return(
         ...
         {product=>{
                 ...
                 <div onClick={()=>{set_dropdowb(dropdown=>!dropdown)}} 
                 style={{backgroundImage: `url(${logo})`}} className={s.header_avatar}>
                 <i className={s.dropbtn} >
                </i>
                 </div>
   ...
   <div className={s.dropdown_content} style={{display: dropdown ? 'block' : 'none' }}>
       <ul>
           <li className={s.profil_text} onClick={()=>{product.openModal();set_dropdowb(dropdown=>!dropdown)}}>
               Увійти 
           </li>
           </ul>
   </div>
                  ...
                  <div className={s.header_all}>
                 <div 
                 onClick={()=>{set_dropdowb(dropdown=>!dropdown)}}
                 style={{backgroundImage: 'url(https://icons-for-free.com/iconfiles/png/512/business+costume+male+man+office+user+icon-1320196264882354682.png)'}} 
   ...
   <div className={s.dropdown_content} style={{display: dropdown ? 'block' : 'none' }}>
          <ul>
           ...
           <li  className={s.profil_text} onClick={()=>{product.log_out();set_dropdowb(dropdown=>!dropdown)}}>
            ...
            <strong className={s.welcome_main_text}>Цифрова трансформація бізнесу в Україні</strong>
            <ProductConsumer>
         {product=>{
                 ...
                 <div className={s.welcome_main_text_hello}>Вітаємо,{product.user[0].login}</div>)}}
            </ProductConsumer>
